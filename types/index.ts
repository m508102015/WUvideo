...
export interface FavoriteItem {
  ...
  // 追劇進度 / 更新檢查
  savedEpisodeCount?: number;   // 上次檢查時的觀看集數（1-based）
  latestEpisodeCount?: number;  // 目前來源最新總集數
  watchedEpisode?: number;     // 目前觀看到的集數（1-based）
  nextEpisodeIndex?: number;   // 下一集的 episode index（0-based）
  unwatchedEpisodeCount?: number; // 尚未觀看的集數
  hasUpdate?: boolean;          // 是否有未看集數
  updateCheckedAt?: number;     // 上次成功檢查時間
}
